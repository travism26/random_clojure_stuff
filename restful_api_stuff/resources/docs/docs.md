...
The `home-routes` handler in the `swag.routes.home` namespace
...
The `home-page` function will in turn call the `swag.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `swag.handler` namespace:
...
Middleware used for development is placed in the `swag.dev-middleware` namespace found in
